...
const io = socketIo(server, {
    cors: {
        origin: "*", // Don't use env during dev; keep it hardcoded
        methods: ["GET", "POST"],
        credentials: true
    }
});
...
const dbConfig = {
    host: 'localhost',
    user: 'akshitdh_freshmart',
    password: 'Super@30',
    database: 'akshitdh_grocery_deliver',
    timezone: 'local',
    charset: 'utf8mb4'
};
...
function verifyToken(token) {
    return jwt.verify(token,  'supersecretjwtkey');
}
...
app.get('/api/landing/page',async (req, res) => {
    ...
    `);
        ...
        // Count of active buyers in last 7 days
        const [activeBuyers] = await dbPool.execute(`
      SELECT COUNT(DISTINCT u.id) as active_buyers_count
      FROM user_sessions us
    ...
    `);
        ...
        const result = {
            ...
            activeBuyersCount: activeBuyers[0].active_buyers_count
        };
