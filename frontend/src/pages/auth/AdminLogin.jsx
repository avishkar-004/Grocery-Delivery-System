...
import {
  ...
  CheckCircle,
  AlertCircle,
  UserCog, // Representing an admin login
  ArrowRight,
...
const AdminLogin = () => {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:3000/api/auth/send-password-reset-otp", {
        ...
        body: JSON.stringify({ email: formData.email.toLowerCase().trim(), role: "admin" }), // Role set to admin
      });
      const data = await response.json();
      ...
      case 'login':
        return (
            <form onSubmit={handleLoginSubmit} className="space-y-6">
              ...
              <span className="bg-clip-text text-transparent bg-gradient-to-r from-purple-600 to-indigo-700"> {/* Admin gradient */}
                Sign In to FreshMarket Admin
              </span>
              </h1>
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <Mail className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5 group-focus-within:text-purple-600 transition-colors duration-200" /> {/* Admin focus */}
                  <input
                      id="email"
                      type="email"
                      placeholder="Email"
                      ...
                      className={`w-full pl-12 pr-4 py-3.5 rounded-full border border-gray-300 dark:border-gray-600 transition-all duration-300 focus:outline-none focus:ring-1 focus:ring-purple-200 text-gray-800 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 bg-gray-100 dark:bg-gray-700 hover:border-purple-400 dark:hover:border-purple-500
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <KeyRound className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5 group-focus-within:text-purple-600 transition-colors duration-200" /> {/* Admin focus */}
                  <input
                      id="password"
                      ...
                      className={`w-full pl-12 pr-12 py-3.5 rounded-full border border-gray-300 dark:border-gray-600 transition-all duration-300 focus:outline-none focus:ring-1 focus:ring-purple-200 text-gray-800 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 bg-gray-100 dark:bg-gray-700 hover:border-purple-400 dark:hover:border-purple-500
              ...
              <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full py-4 mt-6 bg-gradient-to-r from-purple-600 via-fuchsia-600 to-indigo-700 text-white font-bold text-lg rounded-xl shadow-xl hover:shadow-2xl hover:from-indigo-700 hover:to-purple-600 transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center space-x-2 animate-button-pop"
              >
                {isLoading ? (
                    <>
  ...
  return (
      <div className="min-h-screen bg-gradient-to-br from-purple-100 to-purple-50 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-4 sm:p-6 lg:p-8 font-inter overflow-hidden relative transition-colors duration-300">
        {/* Subtle Full-page Background */}
        <div className="absolute inset-0 z-0 bg-gradient-to-br from-purple-100 to-purple-200 dark:from-gray-950 dark:to-gray-850 opacity-90 transition-colors duration-300"></div>
        ...
        <div className="w-full max-w-4xl bg-white dark:bg-gray-800 rounded-3xl shadow-2xl overflow-hidden my-8 flex flex-col md:flex-row z-10 transition-colors duration-300">
          ...
          {/* Left Half: Branding and Welcome - Purple Theme */}
          <div className="md:w-1/2 p-8 flex flex-col items-center justify-center bg-gradient-to-br from-purple-700 to-indigo-800 text-white text-center
          rounded-l-3xl rounded-r-none md:rounded-r-none rounded-b-3xl md:rounded-b-none relative overflow-hidden">
            ...
            <div className="w-28 h-28 bg-white/20 rounded-full flex items-center justify-center mb-6 shadow-xl animate-scale-in-subtle z-10">
              <UserCog className="w-16 h-16 text-white transform rotate-6" /> {/* UserCog icon for admin */}
            </div>
            <h2 className="text-4xl font-extrabold mb-4 text-shadow-lg animate-fade-in-up z-10">
              Welcome to <br />FreshMarket Admin
            </h2>
            <p className="text-lg opacity-90 leading-relaxed max-w-sm mb-8 animate-fade-in z-10">
              Manage the FreshMarket platform with powerful tools.
            </p>

            {/* Home button (Sign Up link not applicable for Admin login typically) */}
            <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3 w-full max-w-sm justify-center items-center z-10 mt-6">
              <button
                  onClick={() => navigate("/")}
                  className="w-full sm:w-auto py-3.5 px-6 border-2 border-white text-white font-bold text-lg rounded-full shadow-lg hover:shadow-xl hover:bg-white hover:text-purple-700 transition-all duration-300 transform hover:scale-105 active:scale-95 flex items-center justify-center space-x-2 group"
              >
                <Home className="w-5 h-5 transition-transform duration-300 group-hover:rotate-6" />
                <span>HOME</span>
              </button>
              {/* Admin login typically doesn't have a direct sign-up from here */}
            </div>
          </div>
