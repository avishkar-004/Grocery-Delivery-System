...
const QuotationManagement = () => {
  ...
  const [discount, setDiscount] = useState(''); // Initialize with empty string for controlled input
  const [notes, setNotes] = useState(''); // Initialize with empty string
  ...
  const fetchAndFilterOrders = useCallback(async (page = 1) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Correctly set totalPages and totalOrders from the 'error' object as per provided API response
        setTotalPages(response.error?.total_pages || 1);
        setTotalOrders(response.error?.total_items || 0);
        setCurrentPage(page);
      } else {
      ...
      if (response.success) {
        ...
        if (isEdit && quotationData) {
          ...
          setQuotationItems(quotationData.items.map(item => ({
            ...
            quantity: item.quantity, // Assuming 'quantity' in quotationData.items maps to 'available_quantity' in form
            unit_type: item.unit_type,
            price_per_unit: item.price_per_unit.toString(),
            available_quantity: item.available_quantity, // Use available_quantity from quotationData
            notes: item.notes || '',
          })));
          ...
          setQuotationItems(response.data.items.map(item => ({
            ...
            product_name: item.product_name,
            requested_quantity: item.requested_quantity,
            quantity: item.requested_quantity, // Default to requested quantity for new quote
            unit_type: item.unit_type,
            price_per_unit: '',
            available_quantity: item.requested_quantity, // Default to requested quantity for new quote
  ...
  const sendQuotation = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (!editingQuotation) {
          // Re-fetch orders to ensure accurate pagination and updated list
          fetchAndFilterOrders(currentPage);
        } else {
          // If editing, no need to remove from list, but maybe refresh current page if order status changed
          fetchAndFilterOrders(currentPage);
        }
        closeQuotationModal();
        setValidationErrors({});
            ...
            <div className="flex items-center space-x-1">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => {
                  ...
                  const showPageNum =
                      ...
                      (pageNum >= currentPage - 2 && pageNum <= currentPage + 2); // Show 2 pages around current

                  if (!showPageNum) return null;

                  // Add ellipsis for skipped pages
                  if (pageNum > 1 && pageNum === currentPage - 3 && totalPages > 5) {
                      return <span key={`ellipsis-prev-${pageNum}`} className="px-2 text-gray-500 dark:text-gray-400">...</span>;
                  }
                  if (pageNum < totalPages && pageNum === currentPage + 3 && totalPages > 5) {
                      return <span key={`ellipsis-next-${pageNum}`} className="px-2 text-gray-500 dark:text-gray-400">...</span>;
                  }
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
                        <div className="flex-1 w-full sm:w-auto mb-3 sm:mb-0"> {/* Added responsive width and margin */}
                          ...
                          <p className="text-sm text-gray-600 dark:text-gray-300 flex items-center">
                            ...
                            {order.distance ? `${order.distance.toFixed(2)} km` : 'N/A'} from you
                          </p>
