...
const BuyerDashboard = () => {
    ...
    // NEW: State for cart toggle functionality
    ...
    // NEW: Function to toggle cart drawer
    const toggleCart = () => {
    ...
    // NEW: Dynamic grid classes based on cart state
    ...
    // NEW: Dynamic category grid classes based on cart state
    const getCategoryGridClasses = () => {
        if (isCartOpen) {
            return "grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4";
        } else {
            return "grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 2xl:grid-cols-10 gap-4";
        }
    };
    ...
    return (
        <div className={`min-h-screen font-inter transition-colors duration-300 ${
            ...
            {/* NEW: Floating Cart Toggle Button - Always visible, fixed position */}
            <button
                onClick={toggleCart}
                className={`fixed top-4 right-4 z-50 bg-green-500 text-white p-3 rounded-full shadow-lg hover:bg-green-600 transition-all duration-300 transform hover:scale-110 ${
            ...
            {/* Main Content Container - Dynamic width based on cart state */}
            <div
                ref={mainContentRef}
                ...
                style={{
                    width: isCartOpen ? 'calc(100% - 350px)' : '100%'
                }}
            >
                {/* Search Bar */}
          ...
          Adding...
            ...
            {/* NEW: Sliding Cart Drawer - Fixed position, slides from right */}
            <div className={`fixed top-0 right-0 h-full w-[350px] bg-white dark:bg-gray-800 shadow-xl transform transition-transform duration-300 z-40 ${
                isCartOpen ? 'translate-x-0' : 'translate-x-full'
            }`}>
                <div className="h-full flex flex-col">
                    {/* Cart Header */}
                    <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
                        <h2 className="text-xl font-semibold text-green-700 dark:text-green-400">Shopping Cart</h2>
                        <button
                            onClick={toggleCart}
                            ...
                            <>
                                ...
                                <div className="flex-1 overflow-y-auto p-4">
                                    <div className="space-y-4">
                                        {cartItems.map((item) => (
                                            <div key={item.cart_item_id} className="flex items-start justify-between border-b border-gray-200 dark:border-gray-700 pb-4">
                                                ...
                                                <div className="flex items-center space-x-2 ml-3">
                                                    <div className="flex items-center space-x-1">
                                                        <button
                                                            ...
                                                            className="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 w-6 h-6 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors disabled:opacity-50 text-sm flex items-center justify-center"
                                                        >
                                                            −
                                                        </button>
                                                        <span className="font-bold text-sm w-8 text-center">{item.requested_quantity}</span>
                                                        <button
                                                            onClick={() => handleUpdateCartItem(item.cart_item_id, item.requested_quantity + 1)}
                                                            className="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 w-6 h-6 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors text-sm flex items-center justify-center"
                                                        >
                                                            +
                                                        </button>
                                                    </div>
            ...
            {/* NEW: Overlay for mobile to close cart when clicking outside */}
            {isCartOpen && (
                <div
