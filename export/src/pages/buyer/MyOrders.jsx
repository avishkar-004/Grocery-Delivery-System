...
// 💡 CREATED THE SOCKET INSTANCE OUTSIDE THE COMPONENT
...
const socket = io(API_BASE_URL, {
    autoConnect: false,
    withCredentials: true,        // ✅ This is required when backend sets credentials: true
    transports: ["websocket"],    // ✅ Optional, but avoids CORS issues with polling fallback
});
...
const MyOrders = () => {
    ...
    // 🚀 NEW useEffect for managing the entire chat socket lifecycle
    useEffect(() => {
        // Only run this effect when the chat modal is open
        if (showQuotationItemDetailsModal && selectedOrder?.id) {
            const token = getAuthToken();

            // Connect the socket with the auth token
            socket.auth = { token };
            socket.connect();

            // 1. EVENT EMITTER: Tell the server we want to join this order's chat room
            socket.emit('join_order_chat', { orderId: selectedOrder.id });

            // 2. EVENT LISTENER: Listen for the initial message history from the server
            const handleMessageHistory = (data) => {
                if (data.orderId === selectedOrder.id) {
                    console.log('Received message history:', data.messages);
                    setQuotationMessages(data.messages || []);
                }
            };
            socket.on('order_messages_history', handleMessageHistory);


            // 3. EVENT LISTENER: Listen for new, real-time messages
            const handleNewMessage = (newMessage) => {
                console.log('Received new message:', newMessage);
                // Use functional update to safely add the new message to the list
                setQuotationMessages(prevMessages => [...prevMessages, newMessage]);
            };
            socket.on('new_order_message', handleNewMessage);

            // 4. CLEANUP FUNCTION: This is crucial. It runs when the modal closes.
            return () => {
                console.log('Leaving order chat, removing listeners.');
                // Remove the specific listeners to avoid them firing for other chats
                socket.off('order_messages_history', handleMessageHistory);
                socket.off('new_order_message', handleNewMessage);
                // Optional: You can emit a "leave" event if you have one on the server
                // socket.disconnect(); // Uncomment if you want to disconnect on modal close
            };
        }
    }, [showQuotationItemDetailsModal, selectedOrder, getAuthToken]);
    ...
    // ✅ UPDATED handleSendMessage function to use sockets
    const handleSendMessage = useCallback(() => {
        ...
        const messageData = {
            ...
            messageType: 'text', // Or handle other types if needed
        };

        // Emit the message to the server via socket
        socket.emit('send_order_message', messageData);

        // Clear the input field immediately
        setNewMessage('');

        // No need to call fetch anymore, the 'new_order_message' listener will handle it
    }, [newMessage, selectedOrder]);
    ...
    const handleViewQuotationDetails = (quotation) => {
        ...
        // fetchQuotationMessages(quotation.id); // 👈 REMOVED THIS LINE
        fetchQuotationItemsDetails(quotation.id); // Kept this one for product details
    };
